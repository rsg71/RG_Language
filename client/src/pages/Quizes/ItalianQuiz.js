...
export default function ItalianQuiz() {
    ...
    async function verifyAnswer(value, enterKeyPressed) {
        if (value === answer) {
            ...
            let res = await API.answerItalianCorrectly(currentWord._id);
    ...
    return (
        ...
        <>
            <Container >
                <DefaultBreadcrumb pageTitle="Italian Quiz" />
                ...
                {loaded && !error && !loading &&
                    <>
                        <Row>
                            <Col className="mb-3">
                                <Link to="/italian"><Button><i className="fas fa-arrow-circle-left"></i> Back</Button></Link>
                            </Col>
                        </Row>
